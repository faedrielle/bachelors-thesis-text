Based on the desired functionality of the platform described in the previous chapter, I was able to create a design for the user interface -- UI of the mobile application.
...
A hamburger menu (or \textit{navigation drawer}) located in the top left corner is nearly inaccessible for the majority of users, who are right-handed and use their thumb to operate the device~\cite{thumb-zone-article}.
...
I chose the tab bar with five tabs: \code{Plans}, \code{Routes}, \code{Map}, \code{Friends}, and \code{Profile}.
...
\section{Profile tab}
After setup, the APP opens on the \code{Profile} tab~(fig.\ref{fig:profile1}), displaying all entered and calculated information on cards, as well as the option to log out, and the currently active device~(US~\ref{US:user-manage-info}, \ref{US:user-log-in}, \ref{US:HRM-device-activate}).
...
On the \code{Fitness tests} screen~(fig.\ref{fig:fitnessTests}), the user can choose between multiple tests, each of which has a tag saying what it tests~(epic~\ref{epic:fit}).
As an example, the \code{6 Minute Walking Test} screen has directions on what the user should do, and after the test, there is a screen with results~(fig.\ref{fig:6mwt}).
...
When we come back to the \code{Profile} screen (fig.\ref{fig:profile1}), we can also manually edit all the information the user has provided.
...
The \code{Devices} screen~(fig.\ref{fig:devices}) contains the active device which can be deactivated (US~\ref{US:HRM-device-activate}, \ref{US:HRM-device-management}), as well as other paired devices, which can be either activated or unpaired.
The user can also pair a new device with the app (opening a list of devices paired with the phone).
Tapping the \code{Activate} button will deactivate the currently active device.
...
\section{Map tab}
The default tab on which the application opens is the \code{Map} tab~(fig.\ref{fig:map}).
...
In the \code{Waypoints} step~fig.\ref{fig:plan-waypoints}), the user picks where they want to start their route, the destination, and all the points through which they want to pass.
...
When the user is satisfied with the waypoints, they tap the \code{Find a route} button.
...
If the user wants to just start the hike without planning for a different day or inviting friends, they can press the \code{GO} button and the APP navigates them,
...
These can be searched for among the user's friends, using the \code{Invite people} button~(fig.\ref{fig:plan-invite-friends}).
...
This screen also contains a checkbox to allow friends join the hike -- whether the hike should be marked as joinable when it is shared to the friends' feed of \code{All plans}~(fig.\ref{fig:plan-all-plans}).

Once the planning is finished, we return to the basic \code{Map} screen, with a snack bar notification telling the user where they can find the planned hike and how many people were invited~(fig.\ref{fig:plan-end}).

The whole planning can be stopped at any point with the \code{Cancel} button, which will trigger a confirmation dialogue, as the user is about to delete all their progress, and after confirming, we return to the \code{Map} tab.
...
\begin{figure}[h!]
    ...
    \caption{Overview of the hike. After pressing the \code{Finish} button, we are back to the \code{Map} screen.}
    \label{fig:plan-end}
\end{figure}

\section{Plans tab}
The \code{Plans} tab is divided into three sections -- \code{All plans}, \code{My plans} and \code{History}.
The default \code{All plans} section~(fig.\ref{fig:plan-all-plans}) is a filterable feed of the users' and their friends' upcoming plans (US~\ref{US:friends-join-hikes}).
...
If the hike is joinable, there is an \code{Ask to join} button (US~\ref{US:friends-join-hikes}); if not, the user can only go back.
...
The \code{My plans} section~(fig.\ref{fig:plan-my-plans}) contains the hikes that the user has planned or joined.
...
Requests to join are shown in this detail; by tapping at the \code{People} section the user can manage the people connected to the hike, as well as allow or disallow people to join the hike (US~\ref{US:friends-join-hikes}).
...
The \code{Start} button will become active on the day of the hike.
...
Tapping on the \code{Route} section will show the route's details, just like in fig.\ref{fig:plan-pick-route}.
...
The \code{History} section~(fig.\ref{fig:plan-history}) of the \code{Plans} tab is the same as the \code{My plans} section, except all the hikes are in the past (US~\ref{US:map-history}).
Each hikes' map contains the planned route, and the real route in case the user did not precisely follow the plan.
This hike can be repeated by pressing the \code{Plan a hike} button and choosing whether to use the originally planned or the actually taken route.
...
\section{Routes tab}
The \code{Routes} tab~(fig.\ref{fig:routes}) contains a filterable list of saved routes (US~\ref{US:map-saved}).
...
\section{Friends tab}
Finally, the last tab is the \code{Friends} tab.
...
By pressing the add button, the user is navigated to the Add friends screen~(fig.\ref{fig:friends}) can also add new friends using a link or social media~(US~\ref{US:friends-add}).
...
Once a hike is started, the view is switched to the \code{Map} tab.
...
If the user wants to see something else than the navigation mode, they can use the \code{Close} button on the top right, or just switch between tabs.
