...
\section{Profile tab}
After setup, the APP opens on the `Profile' tab~(fig.\ref{fig:profile1}), displaying all entered and calculated information on cards, as well as the option to log out, and the currently active device~(US~\ref{US:user-manage-info}, \ref{US:user-log-in}, \ref{US:HRM-device-activate}).
...
On the `Fitness tests' screen~(fig.\ref{fig:fitnessTests}), the user can choose between multiple tests, each of which has a tag saying what it tests~(epic~\ref{epic:fit}).
