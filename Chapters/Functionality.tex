...
where the reason part is optional but recommended.
...
Vendors are discussed in the PoC implementation chapter.

\paragraph*{Acceptance criteria}
\begin{quote}
\begin{itemize}
    \item A popular kind of wearable can communicate with the App in both directions.
\end{itemize}
\end{quote}
...
If the user has no physical impairments, they can have their HR max assessed by a simple age-based formula, such as the one developed by Nes et al., or take one of the existing HR max tests knowing that they are dangerous when not done right (see the chapter on fitness assessment).
...
More research is needed in cooperation with data scientists and physical therapy professionals, in order to develop reliable models for fast difficulty assessment of a route for a specific individual.
...
The only people that can be added need to be users of the App.
...
\section{Notes}
Since the platform needs data from similar users to make estimations on routes, all features except for navigation require an active Internet connection.
Navigation is working with already existing plans and reception in mountainous regions isn't ideal, which is why maps for planned routes should be downloaded.
